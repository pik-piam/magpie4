...
- bilateral:
  ...
  Logical or NULL. If NULL (default), auto-detects: uses bilateral trade
  matrix (ov21_trade) if present, otherwise falls back to net trade. If
  TRUE, requires bilateral data and warns if not found before falling
  back. If FALSE, always uses net trade flows.

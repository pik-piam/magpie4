...
water_avail <- function(gdx, file=NULL, level="reg", sources=NULL, sum=TRUE,digits=0) {
  x<-readGDX(gdx,"ov43_watavail","ov_watavail","ovm_watavail", format="first_found")[,,"level"]
  
  if(!is.null(sources)) x<-x[,,sources]
  x<-dimSums(x,dim=3.2)
  
  if(sum==TRUE) x<-dimSums(x,dim=3.1)

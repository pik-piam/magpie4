...
#' @param attributes dry matter: Mt/ha ("dm"), gross energy: PJ/ha ("ge"), reactive nitrogen: Mt/ha ("nr"),
#' phosphor: Mt/ha ("p"), potash: Mt/ha ("k"), wet matter: Mt/ha ("wm"). Can also be a vector.
...
yields <- function(gdx,
                   ...
                   attributes = "dm",
                   water_aggr = TRUE) { # nolint:object_name_linter
  if (level == "cell") {
    ...
    if (is.null(prod)) {
      warning(
        "Yields cannot be calculated as production function returned NULL! NULL is returned!"
      )
      return(NULL)
    }
    if (products == "pasture") {

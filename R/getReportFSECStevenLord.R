...
#' - Biodiversity (BII)
#' - Population (Millions people)
#' - Global Surface Temperature (deg C)
#' - GDP (PPP)
#' - GHG emissions (CO2, CH4, NO2, as well as totalled and cumulative in CO2eq)
#' - Food system costs
#' - Tau
#'
#' @export
...
getReportFSECStevenLord <- function(magpieOutputDir, reportOutputDir = NULL, scenario = NULL) {
    ...
    message("getReportFSECStevenLord: Collecting population datasets for scenario: ", scenario)
    ...
    gdp_path <- file.path(magpieOutputDir, "../../modules/09_drivers/f09_gdp_ppp_iso.csv")

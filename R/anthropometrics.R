...
anthropometrics<-function(gdx,indicator="bodyheight", age="adults", sex=FALSE,bmi_groups=FALSE, level="iso", final=TRUE,file=NULL,calibrated=TRUE){
  ...
  if (indicator == "bmi_shr") {
    if (bmi_groups == FALSE) {
      stop("bmi_groups should be set to true.")
    }
    if (calibrated == FALSE) {
  ...
  if (bmi_groups == FALSE) {
    ...
    if (bmi_groups != TRUE) {
      stop("bmi_groups has to be TRUE or FALSE")
    }
    if (!indicator %in% c("bmi_shr", "BMI", "bodyweight")) {
      stop("bmi_groups so far only exist for body weight, BMI and bmi_shr")
    }
  }
  ...
  if (age == FALSE) {
    ...
    stop("unknown selection for age")
  }
  if (sex == FALSE) {

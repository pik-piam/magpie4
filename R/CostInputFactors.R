...
#'
#'   \dontrun{
#'     x <- CostInputFactors(gdx)
#'   }
#'

CostInputFactors <- function(gdx,type="annuity",file=NULL,level="cell"){
  
  if(suppressWarnings(is.null(readGDX(gdx,"ov_cost_inv")))){
    
    if (is.null(type)){
    out<-collapseNames(readGDX(gdx,"ov_cost_prod")[,,"level"])
    getNames(out)<-"Overall input costs"
    }else{
  ...
  if(!is.null(type)){
    ...
    kcr<-findset("kcr")
    out<-dimSums(collapseNames(readGDX(gdx,"ov_cost_prod")[,,"level"][,,kcr]),dim=3)+collapseNames(readGDX(gdx,"ov_cost_inv")[,,"level"])
    getNames(out)<-"Input costs (Capital Annuity)"
   ...
   out<-variable+capital
     ...
     getNames(out)<-"Input costs (Capital overall)"
  }
  }else{

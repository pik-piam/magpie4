...
#'
#' @param gdx GDX file
#' @param products products for which factor costs should be reported ("kli", "kres", or "pastures")
...
FactorCosts <- function(gdx, products = "kli", file = NULL, level = "regglo") {
  ...
  if (!is.null(cap_historical_share)) {
    share_calibration <- cap_historical_share[,2010,] - (as.double(cap_share_reg_parameters[,,"slope"]) * log10(gdp_pc_ppp[,2010,]) + as.double(cap_share_reg_parameters[,,"intercept"]))
    capital_share <- cap_historical_share[,seq(1995,2100,5),]
    capital_share[,seq(2015,2100,5),] <- as.double(cap_share_reg_parameters[,,"slope"]) * log10(gdp_pc_ppp[,seq(2015,2100,5),]) + as.double(cap_share_reg_parameters[,,"intercept"]) + share_calibration;

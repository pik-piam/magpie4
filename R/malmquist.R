...
malmquist <- function(gdx, file = NULL) {
  #please add to arguments (we need "reg", "glo" and "regglo"; superAggreate can handle this; see other functions)
  level <- "reg"
  production <- setNames(production(gdx, level = level, product_aggr = TRUE), "production")
  costs <- setNames(costs(gdx, level = level, type = "annuity", sum = FALSE)[, , "Input Factors"], "factorcosts")

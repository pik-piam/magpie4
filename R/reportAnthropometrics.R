...
reportAnthropometrics <- function(gdx) {

  bodyw=bodyweight(gdx,level="regglo")
  getNames(bodyw) <- paste0("Nutrition|Anthropometrics|People ", getNames(bodyw), " (million people)")

  bodyheight = anthropometrics(gdx=gdx,indicator="bodyheight", age="adults", sex=TRUE,bmi_groups=FALSE, level="regglo", final=TRUE,calibrated=TRUE)

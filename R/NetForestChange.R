...
NetForestChange <- function(gdx, file = NULL, level = "cell", lowpass = NULL) {
  ...
  if (any(abs(forestryNet[, -1] - a[, -1]) > 10^-6)) {
    warning("Forestry: Sum of gross changes does not match net change")
  }
  ### secondary forest
  a <- dimSums(readGDX(gdx, "ov35_secdforest", select = list(type = "level")), dim = "ac")
  a <- calcAnnual(a)
  secdfExp <- readGDX(gdx, "ov35_secdforest_expansion", select = list(type = "level")) +
    dimSums(readGDX(gdx, "p35_maturesecdf"), dim = 3) +
    dimSums(readGDX(gdx, "p35_disturbance_loss_primf"), dim = 3) +
    dimSums(readGDX(gdx, "p35_disturbance_loss_secdf"), dim = 3)
  secdfRed <- dimSums(readGDX(gdx, "ov35_secdforest_reduction", select = list(type = "level")), dim = 3) +
    dimSums(readGDX(gdx, "p35_disturbance_loss_secdf"), dim = 3)
  secdfExp <- secdfExp / im_years[, getYears(secdfExp), ]
  secdfRed <- secdfRed / im_years[, getYears(secdfRed), ]
  secdfNet <- secdfExp - secdfRed
  if (any(abs(secdfNet[, -1] - a[, -1]) > 10^-6)) {
    warning("Secondary Forest: Sum of gross changes does not match net change")
  }

  ### primary forest
  a <- land(gdx, types = "primforest", level = "cell")
  a <- calcAnnual(a)
  primfRed <- dimSums(readGDX(gdx, "ov35_primforest_reduction", select = list(type = "level")), dim = 3) +
    dimSums(readGDX(gdx, "p35_disturbance_loss_primf"), dim = 3)
  primfExp <- primfRed
  primfExp[, , ] <- 0
  ...
  if (any(abs(primfNet[, -1] - a[, -1]) > 10^-6)) {
    warning("Primary Forest: Sum of gross changes does not match net change")
  }

  ### combine
  a <- mbind(
    setNames(forestryExp[, , "plant"], "plantExp"),
  ...
  yearsHistory <- years[years > 1995 & years <= 2025]
  yearsFuture <- years[years >= 2025]

  # apply lowpass filter (not applied on 1st time step, applied seperatly on historic and future period)
  if (!is.null(lowpass)) {
    a <- mbind(a[, 1995, ], lowpass(a[, yearsHistory, ], i = lowpass), lowpass(a[, yearsFuture, ], i = lowpass)[, -1, ])
  }

...
#' In addition "climate" for the 3 climate regions tropical, temperate and boreal is available.
  ...
  if (!is.null(a)) {
    if (names(dimnames(a))[[3]] == "man58.land58") {
      ...
      if (sum) {
        ...
        b[, , "degrad"] <- dimSums(a[, , c("crop", "past", "forestry", "unused", "peatExtract")], dim =
                                     3)
    ...
    if (level == "climate") {
      mappingCellClimate <- readGDX(gdx, "p58_mapping_cell_climate")
      a <- dimSums(mappingCellClimate * a, dim = 1)
    } else if (level != "cell") {

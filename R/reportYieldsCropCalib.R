...
reportYieldsCropCalib <- function(gdx, detail = FALSE) {
  ...
  yieldWaterAgg <- function(gdx, water_aggr = TRUE, level_zero_name,
                            tau = FALSE, sum_sep = "+", detail = TRUE) {
    out <- YieldsCropCalib(gdx, file = NULL, level = "regglo", tau = tau)
    if (water_aggr == TRUE) {
    ...
    # The +0.000001 is added as a small area for crops with zero values in fm_croparea.
    # Otherwise yields for begr and betr are zero.
    area <- magpiesort(setYears(superAggregate(readGDX(gdx, "fm_croparea")[, 1995, ] + 0.000001,
                                               aggr_type = "sum", level = "regglo"), NULL))

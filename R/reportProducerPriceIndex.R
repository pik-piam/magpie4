...
reportProducerPriceIndex <- function(gdx, prod_groups = FALSE, level = "regglo") {

  if (prod_groups) {
    #read in data
    primary <- priceIndex(gdx, level = level, products = "kfo", baseyear = "y2020", type = "producer")
    crops <- priceIndex(gdx, level = level, products = "kcr", baseyear = "y2020", type = "producer")
    livestock <- priceIndex(gdx, level = level, products = "kli", baseyear = "y2020", type = "producer")
    bioenergy <- priceIndex(gdx, level = level, products = c("begr", "betr"), baseyear = "y2020", type = "producer")

    #rename
    getNames(primary) <- "Prices|Index2020|Agriculture|Producer|Primary food products (Index 2020=100)"
    getNames(crops) <- paste0("Prices|Index2020|Agriculture|Producer|",
                              reportingnames("kcr"), " (Index 2020=100)", sep = "")
    getNames(livestock) <- paste0("Prices|Index2020|Agriculture|Producer|",
                                  reportingnames("kli"), " (Index 2020=100)", sep = "")
    ...
    pr <- priceIndex(gdx, level = level, products = "kall",
                     product_aggr = FALSE, baseyear = "y2020", type = "producer")
    getNames(pr, dim = 1) <- paste0("Prices|Index2020|Agriculture|Producer|",
                                    reportingnames(getNames(pr, dim = 1)), " (Index 2020=100)")
    ...
    all <-  priceIndex(gdx, level = level, products = "kall", baseyear = "y2020", type = "producer")
    getNames(all) <- paste0("Prices|Index2020|Agriculture|Producer|",
                            reportingnames("kall"), " (Index 2020=100)", sep = "")

    out <- mbind(pr, crops, livestock, all)
  }

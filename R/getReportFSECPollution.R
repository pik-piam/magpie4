...
#' @importFrom madrat toolGetMapping toolConditionalReplace calcOutput
#' @importFrom dplyr %>% group_by summarise
#' @importFrom rlang .data
#' @importFrom mrmagpie calcGridPop_new
#' @examples
#'
#'   \dontrun{
...
getReportFSECPollution <- function(gdx, reportOutputDir = NULL, magpieOutputDir, scenario = NULL) {
  ...
  nbCropland_perAreaHarvested <- toolConditionalReplace(x = nbCropland_perAreaHarvested,
  ...
  nbPasture_perAreaPasture <- toolConditionalReplace(x = nbPasture_perAreaPasture,
                                                     conditions = "!is.finite()", replaceby = 0)
  ...
  nbNonAgLand_perAreaNonAgLand <- toolConditionalReplace(x = nbNonAgLand_perAreaNonAgLand,
                                                         conditions = "!is.finite()", replaceby = 0)
  ...
  nbManureExcretion_perAreaTotalLand <- toolConditionalReplace(x = nbManureExcretion_perAreaTotalLand,
                                                               conditions = "!is.finite()", replaceby = 0)
  ...
  .saveNetCDFReport(nutrientSurplus_unaggregated,
                    file = "nutrientSurplus_anthropogenic_unaggregated", comment = "kg N / ha")
  ...
  pop <- madrat::calcOutput("GridPop_new", aggregate = FALSE)
  ...
  nutrientSurplus_perPopulation <- toolConditionalReplace(x = nutrientSurplus_perPopulation,
                                                          conditions = "!is.finite()", replaceby = 0)
  ...
  .saveNetCDFReport(nutrientSurplus_perPopulation,
                    file = "nutrientSurplus_anthropogenic_perPopulation", comment = "(kg N / ha) / person")

  ######################################
  # S * P
  nutrientSurplus_byPopulation <- nutrientSurplus_noNat * pop
  nutrientSurplus_byPopulation <- toolConditionalReplace(x = nutrientSurplus_byPopulation,
                                                         conditions = "!is.finite()", replaceby = 0)
  ...
  .saveNetCDFReport(nutrientSurplus_byPopulation,
                    file = "nutrientSurplus_anthropogenic_byPopulation", comment = "(kg N / ha) * millions persons")
  ...
  ######################################
  # Planetary boundary --- don't use the avg!
  totalNutrientSurplus_noNat <- nbCropland[,, "Nutrient Surplus"] +
    nbPasture[,, "Nutrient Surplus"] +
    nbManureExcretion[,, "Manure|Manure In Confinements|+|Losses"]

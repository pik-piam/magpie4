...
ResidueUsage <- memoise(function(gdx, level = "reg", products = "kcr", product_aggr = FALSE,
  ...
  if (level %in% c("reg", "regglo", "glo")) {
    ...
    Burn_kcr       <- collapseNames(readGDX(gdx, "ov_res_ag_burn", "ov18_res_ag_burn",
                                            format = "first_found")[, , "level"][, , "dm"])
    Removal_kcr    <- collapseNames(readGDX(gdx, "ov18_res_ag_removal")[, , "level"][, , "dm"])

    Feed_kres      <- dimSums(collapseNames(readGDX(gdx, "ov_dem_feed")[, , "level"][, , kres],
                                            collapsedim = "type"), dim = 3.1)
    ...
    if (products == "kres") {
      ...
      if (any(check != 0)) warning(paste0("Sum over all residue removal options and residue removal in
                                          total are not matching. Non-matching in the range of ", range(check)))
      ...
      if (any(check != 0)) warning(paste0("Sum over all residue usage options and ag-residue biomass in total are
                                          not matching. Non-matching in the range of ", range(check)))
    ...
    Usage          <- ResidueUsage(gdx, level = "reg", products = products, product_aggr = product_aggr,
                                   attributes = attributes, water_aggr = water_aggr)
    ...
    ResidueBiomass <- ResidueBiomass(gdx, level = level, products = products, product_aggr = product_aggr,
                                     attributes = attributes, water_aggr = water_aggr)

...
trade<-function(gdx,file=NULL,level="reg",products = "k_trade",product_aggr=FALSE,attributes="dm",weight=FALSE,relative=FALSE,type="net-exports") {
  ...
  if(any(round(diff,2)<0)) {
    if(any(dimSums(production(gdx,level="glo")[,,readGDX(gdx,"kforestry")],dim=c(1,3))==0)){
      cat("Wood and woodfuel production doesn't exist but is demanded for material.")
    } else {warning("For the following categories, demand EXCEEDS production (????) (on top of balanceflow): \n",paste(unique(as.vector(where(round(diff,2)<0)$true$individual[,3])),collapse=", "),"\n")}
  }
  out<-mbind(

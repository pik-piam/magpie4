...
reportLandUse <- function(gdx) {
  ...
  x <- mbind(x,setNames(a[,,"crop_area"],paste0("Resources|Land Cover|Cropland|+|", reportingnames(getNames(a[,,"crop_area"],dim=1))," (million ha)")))
  x <- mbind(x,setNames(a[,,"crop_fallow"],paste0("Resources|Land Cover|Cropland|+|", reportingnames(getNames(a[,,"crop_fallow"],dim=1))," (million ha)")))
  x <- mbind(x,setNames(a[,,"crop_treecover"],paste0("Resources|Land Cover|Cropland|+|", reportingnames(getNames(a[,,"crop_treecover"],dim=1))," (million ha)")))

...
reportPriceGHG <- function(gdx) {
  ...
  if("emis_source" %in% unlist(strsplit(names(dimnames(a))[[3]],"\\."))) {
    ...
    getNames(co2) <- paste0("Prices|GHG Emission|CO2|",reportingnames(getNames(co2,dim=2))," (US$2017/tCO2)")
    ...
    getNames(n2o) <- paste0("Prices|GHG Emission|N2O|",reportingnames(getNames(n2o,dim=2))," (US$2017/tN2O)")
    ...
    getNames(ch4) <- paste0("Prices|GHG Emission|CH4|",reportingnames(getNames(ch4,dim=2))," (US$2017/tCH4)")

    a <- mbind(co2,n2o,ch4)
  } else {
    a<-a[,,c("co2_c","n2o_n_direct","ch4")]
    getNames(a) <- c("Prices|GHG Emission|CO2 (US$2017/tCO2)",

...
BII <- function(gdx, file = NULL, level = "glo", mode = "auto", landClass = "sum", spatialWeight = NULL,
  ...
  if (mode == "from_grid" || adjusted == TRUE || level %in% c("grid", "iso")) {
    x <- .BIIfromGrid(gdx, level = level, spatialWeight = spatialWeight)
  ...
  if (mode == "auto") {
    ...
    if (!is.null(ov_bv)) {
      ...
      biiCoeffFiles <- c("input/f44_bii_coeff.cs3", "modules/44_biodiversity/bii_btc_apr20/input/f44_bii_coeff.cs3", "modules/44_biodiversity/bv_btc_mar21/input/f44_bii_coeff.cs3")
      sideLayerFiles <- c("input/luh2_side_layers_c200.mz", "modules/44_biodiversity/bii_btc_apr20/input/luh2_side_layers.cs3", "modules/10_land/input/luh2_side_layers.cs3")
      bii_coeff <- suppressWarnings(biiCoeffFiles[min(which(file.exists(biiCoeffFiles)))])
      side_layers <- suppressWarnings(sideLayerFiles[min(which(file.exists(sideLayerFiles)))])
      ov32_land <- readGDX(gdx, "ov32_land", "ov_land_fore", select = list(type = "level"), react = "silent")

      ac <- !is.null(ov32_land) && names(dimnames(ov32_land))[3] == "type32.ac"
...
.BIIfromGrid <- function(gdx, level = level, spatialWeight = spatialWeight) {
  ...
  if (level == "grid") {
    ...
    x <- gdxAggregate(gdx, collapseDim(bii), weight = collapseDim(agg_weight), to = level, absolute = FALSE)
  }

  return(x)
}

.BIImodeMAgPIE <- function(gdx, level = NULL, landClass = NULL, spatialWeight = NULL,
                           adjusted = NULL, bii_coeff = NULL, side_layers = NULL) {
  # read in "biodiversity value" for different land cover classes (unweighted) (in Mha)
  ov_bv <- readGDX(gdx, "ov_bv", select = list(type = "level"), react = "silent")
  if (is.null(ov_bv)) {
    ov_bv <- readGDX(gdx, "ov44_bii", select = list(type = "level"), react = "silent")
  ...
  if (is.null(ov_bv)) {
    stop("No Biodiversity Module in MAgPIE")
  }
  ...
  if (landClass == "all") {
    ...
    land_area <- collapseDim(land(gdx, level = "cell", sum = TRUE))
    bii <- collapseDim(ov_bv / land_area)

    if (level == "cell") {
      x <- bii
    } else {
      x <- superAggregateX(bii, level = level, aggr_type = "weighted_mean", weight = land_area)
    }
  }
...
.BIImodePostprocessing <- function(gdx, level = NULL, landClass = NULL, spatialWeight = NULL,
                                   adjusted = NULL, bii_coeff = NULL, side_layers = NULL) {
  ...
  ov32_land <- readGDX(gdx, "ov32_land", "ov_land_fore", select = list(type = "level"), react = "silent")
  ov35_secdforest <- readGDX(gdx, "ov35_secdforest", select = list(type = "level"))

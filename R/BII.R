...
#' @description calculated the area weighted biodiversity intactness index out of a MAgPIE gdx file
#'
#' @export
#'
#' @param gdx GDX file
...
BII <- function(gdx, file=NULL, level="glo",mode="auto",bii_coeff=NULL,rr_layer=NULL,side_layers=NULL) {
  if(mode=="auto") {
    ...
    if(!is.null(ov44_bv_weighted)) {
      ...
      bii_coeff <- c("input/f44_bii_coeff.cs3","modules/44_biodiversity/bv_btc_mar21/input/f44_bii_coeff.cs3")
      rr_layer <- c("input/rr_layer_c200.mz","modules/44_biodiversity/bv_btc_mar21/input/rr_layer.cs2")
      side_layers <- c("input/luh2_side_layers_c200.mz","modules/44_biodiversity/bv_btc_mar21/input/luh2_side_layers.cs3")

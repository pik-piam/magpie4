...
SOM <- function(gdx, file=NULL, type="stock", reference="actual", level="reg", noncrop_aggr=TRUE, dir=".", spamfiledirectory=""){
  ...
  if(level%in%c("cell","reg","glo","regglo")){
    ...
    lpj_soc_dens  <- collapseNames(read.magpie(paste0(dir,"../../modules/59_som/input/lpj_carbon_topsoil_0.5.mz")), collapsedim = 1)
    ...
    grid_stock    <- gdxAggregate(gdx, som_stock, to="reg", dir = dir, na.rm=TRUE)

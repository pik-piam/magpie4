...
SOM <- memoise(function(gdx, file = NULL, type = "stock", reference = "actual",
  ...
  if (level %in% c("cell", "reg", "glo", "regglo")) {
    ...
    land <- land(gdx, level = "grid", sum = TRUE)
    ...
    land_cell   <- land(gdx, level = "cell", sum = TRUE)
   ...
   if (type == "density")    out  <- som_dens
    ...
    # som_ref     <- gdxAggregate(gdx, setCells(lpj_soc_dens, getCells(land)) * land, to="cell")
    #
    ...
    # som_scaling[is.na(som_scaling)]          <- 0
    # som_scaling[is.infinite(som_scaling)]    <- 0
    # som_scaling <- gdxAggregate(gdx, som_scaling, to="grid", absolute=FALSE)
    #
    ...
    # test            <- gdxAggregate(gdx, som_stock_grid , to="cell")

...
#' 
#' @param gdx GDX file
#' @param type type of indicator. Options: R2, MAE, bias (sum relative error average), error (mean absolute error per hectare)
...
reportFit<-function(gdx,type="error"){
...
# Land types fit/error indicators
  ...
  Unit<- if(type=="error") "Sum Absolute error ha/ha" else if(type=="MAE") "MAE (ha)" else if(type=="bias") "Relative error (%)" else type
...
# Crop types error indicators
   ...
   a[!is.finite(a)]<-0
   getNames(a)<-paste0("Land Cover|Cropland|+|",reportingnames(getNames(a))," (",Unit,")")
   a<-a[,getYears(x),]

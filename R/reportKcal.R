...
reportKcal<-function(gdx, detail = FALSE, level = "regglo") {
  ...
  out <- out[, , unique(getNames(out)[which(dimSums(out, dim = c(1, 2)) !=
                                              0)], keep)]
  ...
  if (!is.null(p15_protein_pc_iso_scp)) {
    ...
    if ("livst_milk" %in% kfo_rd) {
      ...
      oil_scp_milk <- p15_protein_pc_iso_scp[, , "livst_milk"] /
        s15_scp_protein_per_milk * s15_scp_fat_per_milk * fm_nutrition_attributes[, , "oils.kcal"]
      ...
      names(dimnames(oil_scp_milk)) <- names(dimnames(out))
      out <- mbind(out, oil_scp_milk)

      sugar_scp_milk <- p15_protein_pc_iso_scp[, , "livst_milk"] / 
        s15_scp_protein_per_milk * s15_scp_sugar_per_milk * fm_nutrition_attributes[, , "sugar.kcal"]
      sugar_scp_milk <- gdxAggregate(
        gdx = gdx,
        x = sugar_scp_milk,
        weight = "population",
        to = level,
      ...
      names(dimnames(sugar_scp_milk)) <- names(dimnames(out))
      out <- mbind(out, sugar_scp_milk)
    }
    if ("livst_rum" %in% kfo_rd) {
      s15_scp_fat_content <- readGDX(gdx, "s15_scp_fat_content")
      ...
      oil_scp_meat <- p15_protein_pc_iso_scp[, , "livst_rum"] / fm_nutrition_attributes[, , "scp.protein"] *
        (fm_nutrition_attributes[, , "scp.protein"] * s15_scp_fat_protein_ratio_meat - s15_scp_fat_content) * 
        fm_nutrition_attributes[, , "oils.kcal"]
      oil_scp_meat <- gdxAggregate(
        gdx = gdx,
        x = oil_scp_meat,
        weight = "population",
        to = level,
      ...
      names(dimnames(oil_scp_meat)) <- names(dimnames(out))

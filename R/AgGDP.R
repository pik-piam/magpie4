...
AgGDP <- function(gdx, file = NULL, level = "reg") {
  ...
  price_kcr_con <- prices(gdx, level = "reg", products = "kcr", product_aggr = FALSE,
                          attributes = "dm", type = "consumer")
  price_kli_con <- prices(gdx, level = "reg", products = "kli", product_aggr = FALSE,
                          attributes = "dm", type = "consumer")

  ValDemand <- dimSums(demand_kcr * price_kcr_con, dim = 3) +
    dimSums(demand_kli * price_kli_con, dim = 3)

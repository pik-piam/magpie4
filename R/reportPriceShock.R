...
#' @md
reportPriceShock <- function(gdx, level = "regglo") {
  if (!is.null(readGDX(gdx, "p15_delta_kcal_pc", react = "silent"))) {
    kap <- findset("kap")
    before <- Kcal(gdx, level = level, after_shock = FALSE, product_aggr = FALSE, calibrated = FALSE)
    after <- Kcal(gdx, level = level, after_shock = TRUE, product_aggr = FALSE, calibrated = FALSE)
    shock_total_relative <- dimSums(after, dim = 3.1) / dimSums(before, dim =
                                                                  3.1)
    shock_livestock_relative <- dimSums(after[, , kap], dim = 3.1) /
      dimSums(before[, , kap], dim = 3.1)
    shock_others_relative <- dimSums(after[, , "others"], dim = 3.1) /
      dimSums(before[, , "others"], dim = 3.1)
    ...
    shock_expenditure_abs <- FoodExpenditure(gdx = gdx, level = level, after_shock = TRUE) -
      FoodExpenditure(gdx = gdx, level = level, after_shock = FALSE)
    shock_expenditure_relative <- FoodExpenditure(gdx = gdx, level = level, after_shock = TRUE) /
      FoodExpenditure(gdx = gdx, level = level, after_shock = FALSE)

    #shock_hunger_abs=Hunger(gdx = gdx,level=level,after_shock = TRUE)-Hunger(gdx = gdx,level=level,after_shock = FALSE)
    #shock_hunger_relative=Hunger(gdx = gdx,level=level,after_shock = TRUE,share=TRUE)-Hunger(gdx = gdx,level=level,after_shock = FALSE,share=TRUE)

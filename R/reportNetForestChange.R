...
#'
reportNetForestChange <- function(gdx, level = "regglo") {

  x <- NULL
  s32AffPlantation <- readGDX(gdx, "s32_aff_plantation")
  a <- NetForestChange(gdx, level = level)
  ...
  if (!is.null(a)) {
    ...
    if (s32AffPlantation == 0) {
      ...
      x <- mbind(x, setNames(new.magpie(getItems(x, 1.1), getYears(x), NULL, fill = 0, sets = getSets(x)), "Resources|GrossForestLoss|Planted|Plantation|+|CO2-price AR (Mha/yr)"))
    } else if (s32AffPlantation == 1) {
      x <- mbind(x, setNames(new.magpie(getItems(x, 1.1), getYears(x), NULL, fill = 0, sets = getSets(x)), "Resources|GrossForestLoss|Planted|Natural|+|CO2-price AR (Mha/yr)"))
    ...
    if (s32AffPlantation == 0) {
      ...
      x <- mbind(x, setNames(new.magpie(getItems(x, 1.1), getYears(x), NULL, fill = 0, sets = getSets(x)), "Resources|GrossForestGain|Planted|Plantation|+|CO2-price AR (Mha/yr)"))
    } else if (s32AffPlantation == 1) {
      x <- mbind(x, setNames(new.magpie(getItems(x, 1.1), getYears(x), NULL, fill = 0, sets = getSets(x)), "Resources|GrossForestGain|Planted|Natural|+|CO2-price AR (Mha/yr)"))

...
reportCostsInputFactors <- function(gdx) {
  ...
  if (is.null(readGDX(gdx, "p38_capital_mobile_t", react = "silent"))) {
    factor_costs_crops <- superAggregate(readGDX(gdx, "ov_cost_prod", select = list(type = "level"))[, , findset("kcr")], aggr_type = "sum", level = "regglo")
    ...
    # factor costs split by sector (kcr, livestock, residues, pasture)
    ...
    # labor costs split by sector (kcr, livestock, residues, pasture)

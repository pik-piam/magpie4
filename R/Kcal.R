...
Kcal <- function(gdx,
  ...
  out<-gdxAggregate(gdx = gdx,x = out,weight = 'population',to = level,absolute = FALSE)
  ...
  if(any(attributes!="kcal")){
    att=att2=readGDX(gdx=gdx,"fm_nutrition_attributes")[,getYears(out),getNames(out,dim=1)]

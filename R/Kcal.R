...
Kcal <- function(gdx,
  ...
  if (magpie_input == TRUE && after_shock == FALSE) {
    stop("magpie_input and after_shock==FALSE ")
  }
    ...
    if (after_shock == TRUE) {
      ...
      if (magpie_input == FALSE) {
        ...
        if (level %in% c("reg", "glo", "regglo", "GLO")) {
          ...
          stop("magpie_input for kcal cannot be reported below regional level")
        }
      }
    } else if (after_shock == FALSE) {
  ...
  if (any(attributes != "kcal")) {
    att = att2 = readGDX(gdx = gdx, "fm_nutrition_attributes")[, getYears(out), getNames(out, dim =
                                                                                           1)]

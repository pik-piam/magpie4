...
#' @param annual If TRUE, annual values are reported. If FALSE, the values for the whole timestep are reported. If FALSE, avrg has no effect
#' @param avrg If FALSE the annual tc rates of the current period are returned, otherwise the average annual tc rate for the period tbase to tn is returned. tbase defaults to the first timestep (see baseyear)
#' @param baseyear Determines the base year timestep for annual tc calculation. Average tc rates for later timesteps are calculated with respect to baseyear. No tc rates for timesteps before baseyear are returned)
  ...
  if(avrg & annual) {
    ...
    for(r in getRegions(tc)) {
      for(i in 1:nyears(tmp)){
        ...
        tc_avrg[r,i,1] <- (prod(tmp[r,1:i,1]))^(1/sum(l[1:i]))-1
      }
    }
    tc <-tc_avrg
  }

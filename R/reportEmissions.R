...
reportEmissions <- function(gdx) {
  ...
  for (emi in getNames(total,dim=2)){
    prefix<-paste0("Emissions|",reportingnames(emi))
    a<-total[,,emi]
  ...
  x <- mbind(x,setNames(dimSums(a,dim=3),"Emissions|CH4|Agriculture (Mt CH4/yr)"))
  x <- mbind(x,setNames(dimSums(a[,,c("rice")],dim=3),"Emissions|CH4|Agriculture|+|Rice (Mt CH4/yr)"))
  x <- mbind(x,setNames(dimSums(a[,,c("awms")],dim=3),"Emissions|CH4|Agriculture|+|Animal waste management (Mt CH4/yr)"))
  x <- mbind(x,setNames(dimSums(a[,,c("ent_ferm")],dim=3),"Emissions|CH4|Agriculture|+|Enteric fermentation (Mt CH4/yr)"))
